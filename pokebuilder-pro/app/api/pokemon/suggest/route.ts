...
import { sanitizeBuilds, enforceSpeciesClause } from "./_lib/mega-validator";
...
export async function POST(req: NextRequest): Promise<NextResponse<SuggestResponse>> {
  ...
  try {
    ...
    const hasSpeciesClause = normalizedConfig.clauses?.some(
      (c: string) => c.toLowerCase().includes("species clause")
    ) ?? true; // activa por defecto en todos los formatos
    closeNorm();                                       // ✅ sin args
    ...
    for (let attempt = 1; attempt <= MAX_SELECTION_RETRIES + 1; attempt++) {
      ...
      // ✅ validateSelectionResponse usa solo las props que define su tipo
    ...
    // ✅ Species Clause aplicada en post-proceso (no depende del tipo de validateSelectionResponse)
    if (hasSpeciesClause) {
      suggestions = enforceSpeciesClause(suggestions, team);
    }

    closeSanitize();                                   // ✅ sin args
    ...
    const confirmedTeam = [
      ...
      ...suggestions.map((s: any) => {
        ...
        return {
          ...
          tipo1:  poolPoke?.tipo1 || s.tipo1 || "",
          tipo2:  poolPoke?.tipo2 ?? s.tipo2 ?? null,
        };
      }),
    ];
    ...
    return NextResponse.json(
      {
        ...
        report: {
          ...
          strengths:       [],
          recommendation:  "Error interno. Intenta de nuevo.",
        },
        meta: {
          timestamp:                Date.now(),
          configHash:               "error",
